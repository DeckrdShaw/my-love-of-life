const gates = [
  ...
  },
  {
    ...
    answers: ["waited all day", "waiting all day", "waited for my messages", "you waited for my messages"],
    msg: "You waited for my messages the whole day. I have never forgotten that. 🥲"
  },
  {
    q: "What did you say you wanted to do with me?",
    answers: ["hold my hand", "hold hands", "go on this journey together", "hold my hand and go on this journey together"],
...
const letterParts = [
  {
    ...
    text:
      "I have been making this for a long time. I wanted to give it to you on Girlfriend’s Day, but I could not finish it on time. Remember when I wished you at 2:30 a.m.? I was in the middle of making this for you. I am sorry for that, babyy.
...
I know that you love me deeply, even though you do not always express it the way I do. You express it through your actions, through the little things you do, and through the care you quietly give me."
  },
  {
    ...
    text:
      "Thank you for loving me, my babyy. I am working hard for us. Just hold on to me, stay optimistic, and keep your positive energy. I am going to marry you one day, of course I am.
...
I also want you to know that I am going to keep changing myself. I want to become more emotionally mature, understand you better, and give you the love you truly deserve."
  },
  {
    ...
    text:
      "You were right. The heart only pumps blood. But I think love lives in the small things we do every day. It lives in choosing each other, asking about the little things, listening to problems and insecurities, and still rooting for each other.
...
function loadGate() {
  document.getElementById("gateNumber").textContent = `Gate ${gateIndex + 1} of ${gates.length}`;
...
function checkAnswer() {
  ...
  const isCorrect = gates[gateIndex].answers.some((item) => answer.includes(item));

  if (isCorrect) {
    document.getElementById("revealText").textContent = gates[gateIndex].msg;
    showScreen("reveal");
  } else {
    document.getElementById("feedback").textContent = "The gate is shy… try again, Priyatama 🤭";
  }
}
...
function loadLetter() {
  document.getElementById("letterTitle").textContent = letterParts[letterIndex].title;
  document.getElementById("letterText").textContent = letterParts[letterIndex].text;

  const button = document.getElementById("letterButton");
  button.textContent = letterIndex === letterParts.length - 1 ? "Open the final surprise" : "Read the next part";
}
